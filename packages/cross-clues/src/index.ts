...
function isRightDir(dir: Slot["dir"]): boolean {
  return dir.dr === 0 && dir.dc === 1;
}

function dirKeyFromSlot(slot: Slot): number {
  return isRightDir(slot.dir) ? 6 : 8;
}
...
function resolveClueCell(grid: Grid, slot: Slot, dirKey: number): { clueR: number; clueC: number; cluePos: number } {
  ...
  let clueR = slot.r + dr;
  let clueC = slot.c + dc;
  if (clueR < 0 || clueC < 0 || clueR >= grid.rows || clueC >= grid.cols) {
    clueR = slot.r;
    clueC = slot.c;
...
function slotDirName(slot: Slot): "down" | "right" {
  return isRightDir(slot.dir) ? "right" : "down";
}
...
function build02HashComponents(grid: Grid): {
  ...
  const is02Hash = (r: number, c: number): boolean => grid.data[r]?.[c] === "#" && grid.codes[r]?.[c] === 0x02;
...
function buildComponentRectContext(cells: Array<[number, number]>): ComponentRectContext | null {
  ...
  for (let row = 1; row <= height; row += 1) {
    for (let col = 1; col <= width; col += 1) {
      prefix[row][col] =
        fill[row - 1][col - 1] + prefix[row - 1][col] + prefix[row][col - 1] - prefix[row - 1][col - 1];
    }
  }
...
function resolveExpanded02RectangleForClue(
  ...
  clueCells: Array<{ row: number; col: number; slotIds: Set<number>; definitionSlotIds: Set<number> }>,
  ...
  const isSingleDefinitionComponent =
    anchorSlotId !== null && componentDefinitionSlotIds.size === 1 && componentDefinitionSlotIds.has(anchorSlotId);
      ...
      for (let bottom = bottomStart; bottom < localRows; bottom += 1) {
        ...
        for (let left = 0; left <= leftEnd; left += 1) {
          ...
          for (let right = rightStart; right < localCols; right += 1) {
            ...
            if (area < 4) continue;
            if (!allowThinRectangles && (width < 3 || height < 3) && area !== component.cellCount) {
              continue;
            }
...
function countClueCellsInCellSet(
  cellSet: Set<string>,
  clueCells: Array<{ row: number; col: number; slotIds: Set<number>; definitionSlotIds: Set<number> }>,
): number {
  let count = 0;
...
function computeBounds(cells: Array<[number, number]>): PhotoAreaBounds | null {
...
export function buildClueEntries(
  ...
  for (const slot of slots) {
    ...
    const entry: ClueEntry = {
      ...
      clueC,
      cluePos,
      dir: isRightDir(slot.dir) ? "right" : "down",
      word,
      text,
    };
...
export function buildClueLayouts(
  ...
  const expand02Area = typeof options.expand02Area === "boolean" ? options.expand02Area : is02AreaExpansionEnabled();
  const anchorFromSlotIdsWhenNoDefinitions = options.anchorFromSlotIdsWhenNoDefinitions === true;
  ...
  const clueCellsByComponent = new Map<
    number,
    Array<{ row: number; col: number; slotIds: Set<number>; definitionSlotIds: Set<number> }>
  >();
  ...
  const componentExpandedCandidatesById = new Map<number, Map<string, { slotId: number; cellSet: Set<string> }>>();
  ...
  for (const [key, group] of groupByKey) {
    ...
    list.push({
      row: group.row,
      col: group.col,
      slotIds: new Set<number>(group.slotIds),
  ...
  for (const [key, group] of groupByKey) {
    ...
    const orderedEntries = [...group.entries].sort((a, b) => (a.dir === "right" ? 0 : 1) - (b.dir === "right" ? 0 : 1));
    ...
    if (expand02Area && componentId !== undefined) {
      ...
      if (componentContext) {
        ...
        if (!expandedCandidatesByAnchor) {
          ...
          for (const clueCell of clueCells) {
            const candidateSlotIds =
              clueCell.definitionSlotIds.size === 1
                ? [...clueCell.definitionSlotIds]
                : anchorFromSlotIdsWhenNoDefinitions && clueCell.definitionSlotIds.size === 0
                  ? [...clueCell.slotIds]
                  : [];
            const validCandidates: Array<{ slotId: number; cellSet: Set<string> }> = [];
            for (const slotId of candidateSlotIds) {
              const anchorDefinitionSlotIds = new Set<number>([slotId]);
              const clueCellsForCandidate =
                clueCell.definitionSlotIds.size === 1
                  ? clueCells
                  : clueCells.map((item) =>
                      item.row === clueCell.row && item.col === clueCell.col
                        ? { ...item, definitionSlotIds: anchorDefinitionSlotIds }
                        : item,
                    );
              const candidateArea = resolveExpanded02RectangleForClue(
                clueCell.row,
                clueCell.col,
                anchorDefinitionSlotIds,
                componentContext,
                clueCellsForCandidate,
                slotById,
                "anchor",
              );
              ...
              const clueCellCountInCandidate = countClueCellsInCellSet(candidateCellSet, clueCellsForCandidate);
              if (
                definitionSlotIdsInCandidate.size === 1 &&
                definitionSlotIdsInCandidate.has(slotId) &&
                clueCellCountInCandidate === 1
              ) {
                validCandidates.push({ slotId, cellSet: candidateCellSet });
              }
            }
            if (validCandidates.length === 1) {
              const [candidate] = validCandidates;
              if (candidate) {
                expandedCandidatesByAnchor.set(`${clueCell.row},${clueCell.col}`, candidate);
              }
            }
          }
        ...
        const currentCandidate = expandedCandidatesByAnchor.get(currentKey) ?? null;
        let currentCandidateHasOverlapConflict = false;
        if (currentCandidate) {
          for (const [candidateKey, candidate] of expandedCandidatesByAnchor) {
            ...
            if (setsIntersect(currentCandidate.cellSet, candidate.cellSet)) {
              currentCandidateHasOverlapConflict = true;
              break;
            }
          }
        }
        const effectiveDefinitionSlotIds = new Set<number>(group.definitionSlotIds);
        if (
          anchorFromSlotIdsWhenNoDefinitions &&
          effectiveDefinitionSlotIds.size === 0 &&
          currentCandidate &&
          !currentCandidateHasOverlapConflict
        ) {
        ...
        if (anchorDefinitionSlotId !== null) {
          const anchorDefinitionSlotIds = new Set<number>([anchorDefinitionSlotId]);
          ...
          if (expandedArea && expandedArea.length >= 4) {
            ...
            const definitionSlotIdsInExpandedArea = collectDefinitionSlotIdsInCellSet(expandedCellSet, clueCells);
            if (
        ...
        if (clusterArea && clusterArea.length >= 4 && clusterCellSet && !didExpandArea) {
          ...
          if (clusterDefinitionSlotIds.size === 0) {
            ...
            if (effectiveDefinitionSlotIds.has(clusterDefinitionSlotId)) {
              clusterCells = clusterArea;
            }
          }
        }
    ...
    layouts.push({
      ...
      definitionSlotIds: (() => {
        ...
        const hasOverlap = inferred
          ? [...(componentExpandedCandidatesById.get(componentIdForKey) ?? new Map())].some(
              ([candidateKey, candidate]) =>
                candidateKey !== currentKey &&
                candidate.slotId !== inferred &&
                setsIntersect(
                  candidate.cellSet,
                  componentExpandedCandidatesById.get(componentIdForKey)?.get(currentKey)?.cellSet ?? new Set<string>(),
                ),
            )
          : false;
        return inferred != null && !hasOverlap ? [inferred] : [];
      })(),
      areaCells,
      clusterCells,
      text,
...
export function buildPhotoAreaBoundsBySlotId(
  ...
  const photoBoundsByKey = new Map<string, PhotoAreaBounds>();
  for (const clue of photoClues) {
    const photoCells = clue.clusterCells?.length ? clue.clusterCells : clue.areaCells;
    const bounds = computeBounds(photoCells);
    if (!bounds) continue;
    photoBoundsByKey.set(clue.key, bounds);
  }

  const photoBoundsBySlotId = new Map<number, PhotoAreaBounds>();
  for (const clue of clues) {
    const bounds = photoBoundsByKey.get(clue.key);
    if (!bounds) continue;
    for (const slotId of clue.definitionSlotIds) {
      photoBoundsBySlotId.set(slotId, bounds);
    }
  }

  return photoBoundsBySlotId;
}
