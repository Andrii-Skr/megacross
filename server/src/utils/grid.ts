...
// scanSlotsDetailed()   – находим слова-слоты + нумерацию стартов
// scanSlots(grid)       – совместимый wrapper (только slots)
...
const isArrowStart = (at: (r: number, c: number) => Cell, r: number, c: number, dir: Dir): boolean => {
...
export function scanSlotsDetailed(
  grid: Grid,
  options: ScanSlotsDetailedOptions = {}
): ScanSlotsDetailedResult {
  ...
  const minLen = Number.isFinite(minLenRaw) ? Math.max(DEFAULT_MIN_WORD_LEN, Math.trunc(minLenRaw)) : DEFAULT_MIN_WORD_LEN;
  const preferRightOnDualStart = options.preferRightOnDualStart ?? false;

  const slots =
    mode === "arrow"
      ? scanArrowSlots(grid, minLen)
      : scanClassicSlots(grid, minLen, preferRightOnDualStart);
